...
|   attributes	| array of _string_ |    LDAP attributes names to retrieve						|
|   fields	| array of _string_ |    Field names of the previous LDAP attributes. This array must have the same size as *attributes* array. *_id* field can be used to configure the document's id.|
|   index	| _string_     	|     Index name where the documents will be indexed |
|   type	| _string_     	|     Type name of the documents |


Thanks to Jörg Prante for the boilerplate code that comes from the nice "JDBC River Plugin":https://github.com/jprante/elasticsearch-river-jdbc.
